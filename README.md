...
- **Superscript reference detection** — Wraps inline citation numbers (`regions1–8` becomes `regions<sup>1–8</sup>`) and author affiliations with `<sup>` tags. Safely excludes gene names (Ang4, Defa21), figure/table identifiers (fig1, table2), and other non-reference digit patterns.
- **Compound word hyphen handling** — Prefix/suffix-aware line-break rejoining preserves compound words (`microbiota-driven`, `region-enriched`) while correctly joining combining forms (`immunological`, `environmental`). Handles soft hyphens (U+00AD) and PDF replacement characters (U+FFBE/FFFE).
- **Figure annotation filtering** — Detects and removes figure axis labels, gene name lists, and metabolite annotations that leak into body text from PDF figure regions. Uses block detection with strong sentence structure analysis.
