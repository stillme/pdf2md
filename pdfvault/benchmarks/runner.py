"""Benchmark runner — evaluate pdfvault on real papers."""
...
DEFAULT_BENCHMARK_DIR = "~/pdfvault-benchmarks"


def _resolve_local_path(local_filename: str) -> Path:
    """Resolve a benchmark paper's basename to an absolute path under the
    benchmark directory (overridable via the ``PDFVAULT_BENCHMARK_DIR`` env var).
    """
    base = os.environ.get("PDFVAULT_BENCHMARK_DIR", DEFAULT_BENCHMARK_DIR)
...
def _load_pdf(paper: dict, skip_errors: bool = True) -> bytes | None:
    ...
    Resolution order:
      1. ``local_filename`` (a basename) resolved under
         ``$PDFVAULT_BENCHMARK_DIR`` (default ``~/pdfvault-benchmarks``) — if it
         exists, use it.
    ...
    if url:
        r = httpx.get(
            ...
            headers={"User-Agent": "pdfvault-benchmark/1.0"},
        )
...
def _convert_paper(
    ...
    doc = pdfvault.convert(pdf_bytes, **kwargs)
...
def run_benchmarks(
    ...
    """Run pdfvault on benchmark papers and collect results."""
    if papers is None:
