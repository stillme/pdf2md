...
Concurrency: ``ThreadPoolExecutor``. ``pdfvault.convert`` releases the GIL
...
def run_batch(
    ...
    *,
    ...
    callers should leave it ``None`` to use ``pdfvault.core.convert``.
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    if checkpoint_path is None:
        checkpoint_path = output_dir / ".pdfvault-batch.json"
    checkpoint = _Checkpoint(checkpoint_path)

    if convert_fn is None:
        from pdfvault.core import convert as convert_fn  # type: ignore[assignment]
