...
position i to position j is defined as $S \cdot Ti + E \cdot Tj ,$
...
$S \cdot C + E \cdot C to$ the score of the best non-null span
...
the SWAG paper.
sˆi,j = $maxj \geq iS \cdot Ti + E \cdot Tj .$ We predict a non-null
answer when sˆi,j > snull + $\tau ,$ where the thresh￾old $\tau is$ selected on the dev set to maximize F1.
